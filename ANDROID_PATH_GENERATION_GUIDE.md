...
- `maxPlaces` (number) - Максимальное количество мест для посещения. Диапазон: 1-20, по умолчанию 10.
...
Эта ошибка возникает когда алгоритм нашел менее 2 мест в указанной области с учетом всех ограничений (время, дистанция, теги). Решение: увеличьте `totalTime` или `maxDistance`, уменьшите `maxPlaces`, уберите фильтры `tags`, или выберите другую локацию.
...
После оптимизации если получилось меньше 2 мест, возвращается ошибка 400 "Unable to generate route: not enough places found".
